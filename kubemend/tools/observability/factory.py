...
def build_observability_tools(cfg: ObservabilityConfig) -> tuple[ToolSpec, ToolSpec]:
    ...
    if cfg.provider == "datadog":
        datadog = DatadogProvider(
            ...
            api_key=_read_token(cfg.datadog_api_key_file, "observability.datadog_api_key_file"),
            app_key=_read_token(cfg.datadog_app_key_file, "observability.datadog_app_key_file"),
        )
...
def _read_token(path: Path, field_name: str) -> str:
    resolved = Path(path).expanduser()
    if not resolved.exists():
        raise ObservabilityConfigError(
            f"observability.provider is 'datadog' but no credential file at "
