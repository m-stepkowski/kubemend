...
Single provider abstraction (`llm/client.py`) with an Anthropic implementation and a `FakeLLM` for tests. Requirements: (a) prompt caching — cache-control breakpoints after the pinned system+task block and after the stable prefix of the conversation; (b) usage accounting per call (input, cached-input, output tokens → USD via a pricing table in config); (c) two model tiers wired through config: `model.main` (agent turns) and `model.cheap` (compaction, handoff, dev runs).
